...
double **create_text(const char *text)
{
    ...
    for (int i = 0; i < max_chars; i++)
    {
        ...
        {
        case 'A':
            frames[i] = font[CHAR_A];
            break;
        case 'B':
            frames[i] = font[CHAR_B];
            break;
        case 'C':
            frames[i] = font[CHAR_C];
            break;
        case 'D':
            frames[i] = font[CHAR_D];
            break;
        case 'E':
            frames[i] = font[CHAR_E];
            break;
        case 'F':
            frames[i] = font[CHAR_F];
            break;
        case 'G':
            frames[i] = font[CHAR_G];
            break;
        case 'H':
            frames[i] = font[CHAR_H];
            break;
        case 'I':
            frames[i] = font[CHAR_I];
            break;
        case 'J':
            frames[i] = font[CHAR_J];
            break;
        case 'K':
            frames[i] = font[CHAR_K];
            break;
        case 'L':
            frames[i] = font[CHAR_L];
            break;
        case 'M':
            frames[i] = font[CHAR_M];
            break;
        case 'N':
            frames[i] = font[CHAR_N];
            break;
        case 'O':
            frames[i] = font[CHAR_O];
            break;
        case 'P':
            frames[i] = font[CHAR_P];
            break;
        case 'Q':
            frames[i] = font[CHAR_Q];
            break;
        case 'R':
            frames[i] = font[CHAR_R];
            break;
        case 'S':
            frames[i] = font[CHAR_S];
            break;
        case 'T':
            frames[i] = font[CHAR_T];
            break;
        case 'U':
            frames[i] = font[CHAR_U];
            break;
        case 'V':
            frames[i] = font[CHAR_V];
            break;
        case 'W':
            frames[i] = font[CHAR_W];
            break;
        case 'X':
            frames[i] = font[CHAR_X];
            break;
        case 'Y':
            frames[i] = font[CHAR_Y];
            break;
        case 'Z':
            frames[i] = font[CHAR_Z];
            break;
        case ' ':
            frames[i] = font[CHAR_SPACE];
            break;
        case '!':
            frames[i] = font[CHAR_EXCLAMATION];
            break;
        case '.':
            frames[i] = font[CHAR_DOT];
            break;
        default:
            frames[i] = font[CHAR_SPACE];
            break;
...
void show_message(const char *text, RGBColor color, PIO pio, uint sm, double intensity, int speed)
{
    ...
    { // limite arbitrário de segurança
        free(frames);
        return;
    }
 ...
 * Linha 0 (LEDs 0-4) - SEMPRE verde (base)
 * Linha 1 (LEDs 5-9) - verde (intensidade >= 1)
 * Linha 2 (LEDs 10-14) - amarelo (intensidade >= 2)
 * Linha 3 (LEDs 15-19) - amarelo (intensidade >= 3)
 * Linha 4 (LEDs 20-24) - vermelho (intensidade >= 4)
 * @param intensity Intensidade do som (0 a 5)
 * @param avg Valor médio do som (para debug)
 * @param pio Instância PIO
 * @param sm State machine
 */
void led_matrix_update(uint8_t intensity, float avg, PIO pio, uint sm) {
    // Cores pré-definidas (valores entre 0.0 e 1.0)
    const RGBColor green = {0.0, 1.0, 0.0};    // Verde
    const RGBColor yellow = {1.0, 1.0, 0.0};   // Amarelo
    const RGBColor red = {1.0, 0.0, 0.0};      // Vermelho
    const RGBColor off = {0.0, 0.0, 0.0};      // Desligado

    // Atualiza cada LED individualmente
    for (int i = 0; i < NUM_LEDS; i++) {
        RGBColor color = off; // Padrão: desligado
        
        // Linha 0 (sempre verde)
        if (i < 5 && intensity >= 1) {
            color = green;
        }
        // Linha 1 (verde se intensidade >= 1)
        else if (i < 10 && intensity >= 4) {
            color = green;
        }
        // Linha 2 (amarelo se intensidade >= 2)
        else if (i < 15 && intensity >= 8) {
            color = yellow;
        }
        // Linha 3 (amarelo se intensidade >= 3)
        else if (i < 20 && intensity >= 14) {
            color = yellow;
        }
        // Linha 4 (vermelho se intensidade >= 4)
        else if (i < 25 && intensity >= 18) {
            color = red;
        }
    ...
    // Debug
    printf("Intensidade: %d | Media: %.4f V\n", intensity, avg);
}